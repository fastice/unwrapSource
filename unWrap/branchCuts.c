...
static void balanceRegionsPass2(regionList *regions, regionList *topRegion,
                                unwrapImageStructure *unwrapImage, int32_t netCharge);
...
static void cancelBox(unwrapImageStructure *unwrapImage,
                      int32_t r, int32_t a, int32_t boxSize);
static void cancelResidues(unwrapImageStructure *unwrapImage, int32_t nMatch,
                           int32_t nMax, int32_t aResidues[][2], int32_t bResidues[][2]);
static regionList *buildRegionList(regionList *regions,
                                   unwrapImageStructure *unwrapImage, int32_t r, int32_t a);
static void addRegionPoints(regionList *regions,
                            unwrapImageStructure *unwrapImage, int32_t r, int32_t a);
static regionList *nearestRegion(regionList *regions1,
                                 regionList *regions2,
                                 int32_t *rMin1, int32_t *aMin1,
                                 int32_t *rMin2, int32_t *aMin2, int32_t *dMin, int32_t type);
static int32_t distanceBetweenRegions(regionList *regions1,
                                      regionList *regions2,
                                      int32_t *rMin1, int32_t *aMin1,
                                      int32_t *rMin2, int32_t *aMin2);
static int32_t distanceToBorder(regionList *regions,
                                unwrapImageStructure *unwrapImage,
                                int32_t *rMin, int32_t *aMin, int32_t *rB, int32_t *aB);
static void connectRegionPts(regionList *regions,
                             unwrapImageStructure *unwrapImage,
                             int32_t *charged1, int32_t pt, int32_t dMax);
static void sortRegions(regionList *regions);
static void sortRegionPts(int32_t n, rList *ra);
static int32_t comparePts(rList r1, rList r2);
        ...
        for (r = 0; r < unwrapImage->rangeSize - 1; r++)
        {
            ...
            if (unwrapImage->bCuts[a][r] & RESIDUE)
            {
                ...
                if (count <= unwrapImage->residueDensity)
                    ...
                    unwrapImage->bCuts[a][r] &= HIGHDENSITY;
            }
        }
}
...
static void balanceResidues(unwrapImageStructure *unwrapImage)
{
    typedef int32_t(*coordinates)[3];
    int32_t netResidues, nResidues;
    int32_t residue;
    coordinates coord;
    int32_t d, minD;
    int32_t i, j, r;
    int32_t i1, i2, j1, j2, iMin, jMin, cMin;
    netResidues = unwrapImage->nPlus - unwrapImage->nMinus;
    if (netResidues < 0)
        residue = NRESIDUE;
    else if (netResidues > 0)
        residue = PRESIDUE;
    else
        return;

    nResidues = (unwrapImage->nLeft - abs(netResidues)) / 2 + abs(netResidues);
    coord = (coordinates)malloc(3 * nResidues * sizeof(int32_t));
    /*
    ...
    */
    ...
    for (r = 0; r < abs(netResidues); r++)
    {
        ...
        if (iMin == minD)
        { /* Compute coordinates of branch cut */
            i1 = 0;
            i2 = iMin;
            j1 = jMin;
            j2 = jMin;
...
static void dilateResidues(unwrapImageStructure *unwrapImage)
{
    ...
    int32_t count = 0;
    int32_t iMin, iMax, jMin, jMax;
    int32_t boxLeft, boxRight;
    int32_t blockSize;
    blockSize = unwrapImage->blockSize;
    boxRight = (int32_t)(blockSize / 2);
    boxLeft = boxRight;
    if ((blockSize % 2) == 0)
        boxLeft--;
    /*
...
static regionList *regionData(unwrapImageStructure *unwrapImage)
{
    ...
    */
    ...
    if (unwrapImage->mask != NULL)
        fprintf(stderr, "MASK\n");
    if (unwrapImage->mask != NULL)
        maskRegionPoints(regions, unwrapImage);
...
{
    int32_t dBorder1, dBorder2, dNearest;
    int32_t rBorder, aBorder, rBorder1, aBorder1;
    int32_t rTmp, aTmp, rTmp1, aTmp1;
    int32_t r1, a1, r2, a2;
    regionList *tmp, *tmp1;
    /*
    ...
    if (region1->distances != NULL)
    {
        for (tmp = topRegion; tmp != NULL; tmp = tmp->next)
        {
            ...
            if (tmp->nTotal >= 0)
                ...
                d2 = LARGEINT;
            if (d2 < d1)
            { /* the new region is closer so update list */
...
static void balanceRegionsPass2(regionList *regions, regionList *topRegion,
                                ...
                                int32_t netCharge)
{
    int32_t dBorder = LARGEINT, d, dNearest;
    int32_t rBorder, aBorder, rBorder1, aBorder1;
    int32_t rTmp, aTmp, rTmp1, aTmp1;
    int32_t r1, a1, r2, a2;
    int32_t charge;
        ...
        */
        ...
        while (regions->charge != 0)
        {
            ...
            */
            tmp = nearestRegion(regions, topRegion, &r1, &a1, &r2, &a2, &dNearest, IGNORECHARGE);
            if (tmp == NULL)
                fprintf(stderr, "Null tmp in balanceRegions pass 2\n");
                             ...
                             unwrapImageStructure *unwrapImage)
{
    ...
    if ((regions->nTotal > 1) && (regions->mask == FALSE))
    {
        ...
        /*fprintf(stderr,"%i\n",regions->nTotal * sizeof(int32_t));*/
        charged1 = (int32_t *)malloc(regions->nTotal * sizeof(int32_t));
    ...
    if (regions->next != NULL)
        regionsDrawLines(regions->next, unwrapImage);
}
...
static void cancelBox(unwrapImageStructure *unwrapImage,
                      int32_t r, int32_t a, int32_t boxSize)
{
    int32_t pResidue[1000][2], nResidue[1000][2];
    int32_t i, j;
    int32_t nPlus = 0, nMinus = 0, charge, nMatch, nMax;
    int32_t iMin, iMax, jMin, jMax;
    /*
...
static void cancelResidues(unwrapImageStructure *unwrapImage, int32_t nMatch,
                           int32_t nMax,
                           int32_t aResidues[][2], int32_t bResidues[][2])
{
    int32_t i, j;
    int32_t nLeft, dMin, jMin, d1;
    /*
                                   ...
                                   unwrapImageStructure *unwrapImage, int32_t r, int32_t a)
{
    ...
    { /* If found, update */
        ...
        {
            if (unwrapImage->bCuts[a][r] & PRESIDUE)
                (regions->charge)++;
            else
                (regions->charge)--;
        }
                                 ...
                                 int32_t *rMin2, int32_t *aMin2, int32_t *dMin, int32_t type)
{
    ...
    int32_t r1, r2, a1, a2, d;
    int32_t lookUp = FALSE;
    *dMin = LARGEINT;
    /*
...
static regionList *nearestRegionOld(regionList *region1, regionList *region2,
                                    int32_t *rMin1, int32_t *aMin1, int32_t *rMin2, int32_t *aMin2, int32_t *dMin, int32_t type)
{
    regionList *tmp;
    int32_t r1, r2, a1, a2, d;
    int32_t lookUp = FALSE;
    ...
    /*fprintf(stderr,"c %%d %d\n",(int32_t)region1, (int32_t)region2->next);*/
    tmp =
        nearestRegion(region1, region2->next, rMin1, aMin1, rMin2, aMin2, dMin, type);
    /*    fprintf(stderr,"d\n");*/
    if (d < *dMin)
    { /* Is this element closer than those further down list */
        /*
                                      ...
                                      int32_t *rMin2, int32_t *aMin2)
{
    ...
    */
    ...
    if (np1 < 0 || np2 < 0)
        error("distanceBetweenRegions: invalid region");
    ...
    for (i = 0; i < np1; i += iInc)
    { /* Distance of 1 to 2's */
        r1 = tmp1[i].r;
        a1 = tmp1[i].a;
...
{
    int32_t i;
    int32_t dMin = LARGEINT, dLeft, dRight, dUp, dDown, d;
    ...
    np = regions->nTotal;
    if (np < 0)
        error("distanceToBorder: Invalid region %i %i %i",
              regions->label, regions->charge, regions->nTotal);
    left = unwrapImage->left;
    right = unwrapImage->right;
    up = unwrapImage->up;
    down = unwrapImage->down;
    tmp = regions->pts;
    for (i = 0; i < np; i++)
    { /* Distance from + to border */
        r = tmp[i].r;
        a = tmp[i].a;
        dLeft = abs(r - left);
        dRight = abs(r - right);
                                  ...
                                  unwrapImageStructure *unwrapImage)
{
    ...
    for (k = 0; k < regions->nTotal; k++)
    {
        ...
        for (i = iMin; i <= iMax; i++)
            for (j = jMin; j <= jMax; j++)
                unwrapImage->bCuts[a + i][r + j] |= BRANCHCUT;
    }
    return;
}
...
{
    if (regions->nTotal > 1)
        sortRegionPts(regions->nTotal, regions->pts);
    if (regions->next == NULL)
        return;
...
static void sortRegionPts(int32_t n, rList *ra)
{
    ...
    for (;;)
    {
        ...
        while (j <= ir)
        {
            if (j < ir && (comparePts(ra[j], ra[j + 1]) == TRUE))
                ++j; /* *** */
...
{
    int32_t t, distance;
    int32_t xerr = 0, yerr = 0, deltaX, deltaY;
    int32_t incX, incY;
    /*
