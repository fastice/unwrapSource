                           ...
                           float omegaR, float omegaA)
{
    ...
    rCount = (int *) malloc(complexImage->header.rs * sizeof(int));
    for(i=0; i < complexImage->header.rs; i++) rCount[i] = 0;
    aCount = (int *) malloc(complexImage->header.as * sizeof(int));
    for(i=0; i < complexImage->header.as; i++) aCount[i] = 0;
...
*/
          if( (abs(rowA[j].r) < 1) && (abs(rowA[j].i) < 1) ) { 
               ...
               pr = cos(phaseRamp); pi = sin(phaseRamp);
               unwrapImage->phase[i][j] = (float) atan2(pi,pr);
               rCount[j]++; aCount[i]++; /* Counters for finding edge */
          } else { /* Compute phase */
...
*/
    ...
    complexImage->header.rs) {maxLeftj++; i++;}
    i = 0;
    while(aCount[i] >= 0.9*complexImage->header.rs && i <
    complexImage->header.as) {maxDowni++; i++;}
    i = complexImage->header.rs-1;
    while(rCount[i] >= 0.9*complexImage->header.as && i > 0){minRightj--; i--;}
    i = complexImage->header.as-1;
    while(aCount[i] >= 0.9*complexImage->header.rs && i > 0) {minUpi--; i++;}
...
*/
    ...
    if(unwrapImage->mask != NULL) {
        for(i=0; i < complexImage->header.as; i++) free(mask[i]);
        free(mask);
    }
    return;
}  
