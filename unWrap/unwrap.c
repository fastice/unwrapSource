...
*/
 ...
 static void readArgs(int argc, char *argv[], 
                      ...
                      int *residueDensity, int *noInterpolate,char **mask,
                      int *noCenter,long int *thresh,char **labelFile,
   ...
   static void doStats(int *nDiff, double *diff, double *diffSq,double *mean,double *errorThresh,
       inputImageStructure inputImage,unwrapImageStructure *unwrapImage,
       double bnc, double dbn, double bpc, double dbp,
       double thetaC, double ReH, double Re, Offsets offsets,double thresh,int *refLabel,double ambThresh);
...
*/
    ...
    if(offsetsFile !=NULL) fprintf(stderr,"Using Offsets\n%s\n%s\n%s\n",
...
*/ 
    ...
    } else unwrapImage.fpReport=NULL;

    if(unwrapImage.mask != NULL ) {
      fprintf(stderr,"USING MASK IMAGE %s\n",mask);
      if(unwrapImage.fpReport != NULL) fprintf(unwrapImage.fpReport,";  Masked with %s\n",mask);
    }
/* 
...
*/
    if(unwrapImage.mask != NULL ||
      (unwrapImage.nPlus > 0) || 
      (unwrapImage.nPlus > 0) ) branchCuts( &unwrapImage );
/*
   Unwrap phase
*/
...
*/
    if(labelFile != NULL)  {
        ...
        for(i=0; i < azimuthSize; i++)  
           fwriteBS(unwrapImage.labels[i], rangeSize * sizeof(long int),1, fp,INT32FLAG);
    }
                   ...
                   char *geodat, char *baselineFile,double ambThresh)
{
   ...
   */
   nDiff=(int *)malloc(sizeof(int)*(unwrapImage->nLabels+1));
   ...
   for(i=0; i <= unwrapImage->nLabels; i++) {
       nDiff[i]=0; diff[i]=0.0;diffSq[i]=0.;
       errorThresh[i]=1e30;
       mean[i]=0;
   }
   ...
   for(i=0; i < inputImage.azimuthSize; i++) {
	 ...
	 /* Only change labels that are valid */
	    ...
	    if(mean[unwrapImage->labels[i][j]] > (-LARGEINT+10000)) {
              ...
              if(correction > 0) intCorrection = (int)(correction+0.5);
              else intCorrection = (int)(correction-0.5);
       ...
       double thetaC, double ReH, double Re, Offsets offsets,double thresh,int *refLabel, double ambThresh)
{
   ...
   int i,j;
   double bn,bp,bsq;
   int labelCount, currentLabel,maxCount;
   double theta,thetaD;
   double delta,phase,deltaOffset,scalePhase;
   double range; 
   float tmp,tmp1,tmp2;
   int totalGood, maxLabel,maxGood;
   FILE *fp,*fp1,*fp2;
   float zero;
   zero=0.0;
   ...
   for(i=0; i < offsets.na; i++) {
      ...
      for(j=0; j < offsets.nr; j++ ) {
	    ...
	    /* Compute flat earth phase */
              ...
              } else phase=-(float)LARGEINT;
              /* Convert  valid phase to meters */
              if(phase > (-LARGEINT + 10000)) delta =  delta  + scalePhase * phase;
              else delta=-(double)LARGEINT;
              ...
              } else deltaOffset=-(double)LARGEINT;
   ...
   */
   ...
   for(i=0; i <= unwrapImage->nLabels; i++) {
     if(nDiff[i] > 1000) {
         ...
         } else { totalGood+= unwrapImage->labelCount[i]; maxGood=max(maxGood,unwrapImage->labelCount[i]); }
	  maxLabel=max(maxLabel,unwrapImage->labelCount[i]);
         if(nDiff[i] > maxCount) {
             maxCount=nDiff[i];
             *refLabel=i;
         }
	 if(unwrapImage->fpReport != NULL)  fprintf(unwrapImage->fpReport,"%10i %10i %10.2f %% %10i %10.5f %10.5f %10.5f %10.5f\n",i,unwrapImage->labelCount[i],
...
*/
    ...
    for(i=0; i < na; i++) offsets->dr[i]=&(fBufR[i*nr]);
    /*
        Read files
     */
  ...
  static void readArgs(int argc, char *argv[], int *rangeSize, int *azimuthSize,
                       float *omegaA, float *omegaR,int *blockSize, int *residueDensity, int *noInterpolate,char **mask,
                       int *noCenter,long int *thresh,char **labelFile,     char **offsets, char **geodat, char **baselineFile,
                      double *ambThresh, char **reportFile)
{
   int filenameArg;
   char *argString;
   ...
   if( argc < 1 || argc > 25 ) usage();        /* Check number of args */ 
