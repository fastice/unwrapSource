                            ...
                            float *dR1, float *dA1, int modeFlag)
{
    ...
    int rInt, aInt;          /* Integer parts of shifts */
...
*/
    ...
    if(modeFlag == INIT) {
        shifts->columnShift = (int *) malloc(RangeSize*sizeof(int));         
        shifts->rowShift =    (int *) malloc(RangeSize*sizeof(int));
        shifts->aKernel =     (int *) malloc(RangeSize*sizeof(int));         
        shifts->rKernel =     (int *) malloc(RangeSize*sizeof(int));
/*
    Initialize kernel
*/
        initKernel(kernel);
...
*/
        ...
        if( (float) fabs( (double) rFract ) < 0.51 * DSHIFT ) rFract = 0.0;     
        else if( rFract < 0 ) { /* Always use neg fract shift */
            rInt--;
            rFract = -(1.0 + rFract);
        } else rFract = -rFract;
        ...
        if(shifts->rKernel[i] >= NKERNEL) shifts->rKernel[i] = NKERNEL-1;
/*
    Compute Azimuth shifts
*/     
        aInt = (int) deltaA;
        aFract = deltaA - (float)aInt;   
        if( fabs( (double) aFract ) < 0.51 * DSHIFT ) aFract = 0.0;
        else if( aFract < 0 ) {  /* Always use neg fract shift */
            aInt--;
            aFract = -(1.0 + aFract);
        } else aFract = -aFract;
        shifts->rowShift[i] =    -aInt;    /* Row, azimuth */   
        shifts->aKernel[i] = (int)((-aFract+ 0.5 * DSHIFT) * NKERNEL);
        if(shifts->aKernel[i] >= NKERNEL) shifts->aKernel[i] = NKERNEL-1;
/*
...
*/
     if(firstDeltaR > 0) shifts->startRange=(int)firstDeltaR + 1;
     else shifts->startRange = 0;
     if(firstDeltaR < 0) 
         shifts->endRange = (int)(imageA->header.rs + firstDeltaR);
     else shifts->endRange = imageA->header.rs; 

/*
   Compute start and end azimuth
*/
     if(maxAzimuth > 0) shifts->startAzimuth = (int) maxAzimuth + 1;
     else shifts->startAzimuth = 0;
           
     if(minAzimuth < 0) shifts->endAzimuth = 
          (int)(imageA->header.as + minAzimuth);
     else shifts->endAzimuth = imageA->header.as;
    ...
    static void initKernel(ers1Kernel *kernel)
{
    ...
    *kernel = (ers1Kernel) &((*kernel)[0][KERNELSIZE/2]); 
    for(shift = 0; shift < 1.0; shift += DSHIFT) {
        for(i = 0; i <= 16; i++ ) {  /* Load freq values of shift */
