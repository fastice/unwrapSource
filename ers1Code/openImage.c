...
{
    FILE *fp;
    int nBufferRows; /* Number of rows in a buffer */
    int pixelSize;

    if( rwFlag == READIMAGE) { /* Open image for read */
        if(filename == NULL) fp = stdin;
        else fp = fopen(filename,"r");
        if(fp == NULL) error("*** openImage: Error opening %s ***\n",filename);
    ...
    } else error("*** openImage: Invalid mode flag ***\n");
