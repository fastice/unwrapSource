...
void fillBuffer(ers1GenericImage *image)
{
    ...
    FILE *fp;
    int32_t firstRow;           /* First row to read in file, from current - overlap */
    int32_t nRows;              /* Number of rows to read from file */
    int32_t nBufferRows;        /* Number of rows not including overlap regions */
    int32_t nOver;              /* If end of image, number of overlap rows to 0 pad */
    int32_t beginSecondOverlap; /* Byte offset begining of second overlap region */
    int32_t nRowsLeft;          /* Number of rows from current to end of image */
    int32_t pixelSize;          /* Pixel size for image type */
    int32_t imageType;          /* Image type lable */
    int32_t nBytes;             /* Number of bytes to operate on */
    int32_t nPixels;            /* Number of pixels to read into buffer */
    int32_t n;
    int32_t i; /* LCV */
    int32_t bstype;
    ...
    */
    ...
    if ((firstRow + nRows) > header->as)
    {                                            /* If overlap past endof img */
        ...
        if (firstRow == 0)
            tmpBuffer = (char *)image->image;
        else
            tmpBuffer = imageBuffer;
