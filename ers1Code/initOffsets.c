                 ...
                 ers1ComplexImage *imageA)
{
    ...
    int32_t rInt, aInt;   /* Integer parts of offsets */
    ...
    offsets->columnLeft = (int32_t *)malloc(RangeSize * sizeof(int32_t));
    offsets->columnRight = (int32_t *)malloc(RangeSize * sizeof(int32_t));
    offsets->rowUp = (int32_t *)malloc(RangeSize * sizeof(int32_t));
    offsets->rowDown = (int32_t *)malloc(RangeSize * sizeof(int32_t));
    ...
    */
    ...
    if (offsetFile != NULL)
        readShifts(offsetFile, dR, dA, omega);
    for (i = 0; i < RangeSize; i++)
    {
        if (offsetFile != NULL)
        { /* Range independent offsets */
            deltaR = dR[i];
            deltaA = dA[i];
        }
        else
        ...
        */
        if (deltaR > 0)
            t = 1.0 - rFract; /* Interpolation constants */
        else
            t = -rFract;
        if (deltaA > 0)
            u = 1.0 - aFract;
    ...
    */
    if (firstDeltaR > 0)
        offsets->startRange = (int32_t)firstDeltaR + 1;
    else
        offsets->startRange = 0;
    if (firstDeltaR < 0)
        offsets->endRange = (int32_t)(imageA->header.rs + firstDeltaR);
    else
        offsets->endRange = imageA->header.rs;

    /*
       Compute start and end azimuth
    */
    if (maxAzimuth > 0)
        offsets->startAzimuth = (int32_t)maxAzimuth + 1;
    else
        offsets->startAzimuth = 0;

    if (minAzimuth < 0)
        offsets->endAzimuth =
            (int32_t)(imageA->header.as + minAzimuth);
    else
        offsets->endAzimuth = imageA->header.as;
